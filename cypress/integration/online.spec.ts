...
describe('Connecting to server', () => {
	it('should have all moods available', () => {
		...
		cy.window().then({timeout: 10000}, (win) => {
			...
			cy.fixture('expected_soundsets.json')
				.then(expected => {
	...
	it('should show all available moods in playlist', () => {
		...
		cy.get('#playlists .syrin-set option:selected')
			.should('have.text', 'Call of Cthulhu - Masks of Nyarlathotep: Part 1 - Peru: New acquaintances');
		...
		cy.get('#playlists .syrin-mood option:selected')
			.should('have.text', 'The professors home');
	});
	...
	it('should play selected mood', () => {
		...
		cy.get('[title="Play Mood"] > .fas')
			.should('be.visible')
			.click();

		cy.get('.notification')
			.contains('SyrinControl | Playing')
			.should('be.visible');

		cy.get('.playlist-header > .syrin-controls > [title="Stop Mood"] > .fas')
			.should('be.visible');
	});
...
function goToPlaylists() {
	cy.get('[title="Audio Playlists"] > .fas', { timeout: 10000 })
		.should('be.visible')
		.click();
}
...
function setAuthToken() {
	...
	cy.wait(100);
	cy.get('.active > .settings-list > :nth-child(2) > .form-fields > input')
		.should('have.value', Cypress.env('authToken'));
}

function saveConfig() {
	cy.get('.sheet-footer > [type="submit"]')
		.should('be.visible')
		.click();
}
