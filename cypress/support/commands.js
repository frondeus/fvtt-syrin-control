...
Cypress.Commands.add('login', () => {
    ...
    cy.get('select[name="userid"] option:nth-child(2)')
      .then($opt => {
        cy.request({
            method: 'POST',
            url: '/join',
...
Cypress.Commands.add('gotoModuleSettings', () => {
	cy.get('[title="Game Settings"] > .fa-cogs', { timeout: 10000 })
		.should('be.visible')
		.click();

	cy.get('[data-action="configure"]')
		.should('be.visible')
		.click();

	cy.get('.sheet-tabs > [data-tab="modules"]')
		.should('be.visible')
		.click();
});
  ...
  cy.window().then(options, (win) => {
    ...
    cy.fixture(name)
      .then(expected => {
        ...
        setTimeout(() => { win.game.syrinscape.refresh(); }, 500);
      });
