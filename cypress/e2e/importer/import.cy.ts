describe('importer.import', () => {
  ...
  it('should be checkbox to import all visible soundsets', () => {
    ...
    cy.get('@sounds')
      .its(1)
      .its('name').should('eq', 'My mood in kitchen');
    ...
    cy.get('@sounds')
      .its(1)
      .its('flags.syrinscape.mood').should('eq', 4321);
  });
  ...
  it('should be checkbox to import whole soundset', () => {
    ...
    cy.get('@playlists')
      .its('sounds').then(s => Array.from(s.values())).as('sounds');
